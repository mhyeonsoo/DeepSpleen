function [means,diffs,meanDiff,CR,linFit] = BlandAltman(var1, var2, flag)
    ...
    if flag ~= 0
        ...
        if flag > 1
            ...
            plot(means,zeros(1,length(means)),'b'); %%%plot zero     
            plot(means, ones(1,length(means)).*CR(2),'r:'); %%%plot the lower CR
       
            UL = sprintf('+ 1.96 SD: %f',meanDiff+1.96*sdDiff);
            M = sprintf('Mean: %f',meanDiff);
            LL = sprintf('- 1.96 SD: %f',meanDiff-1.96*sdDiff);
            legend('Volume(cc)',UL,M,LL);
        end
        if flag > 2
