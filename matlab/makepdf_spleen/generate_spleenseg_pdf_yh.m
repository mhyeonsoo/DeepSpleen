function generate_spleenseg_pdf_yh(rawfn, segfn, pdffn, cmap,...
...
small_pic1 = imresize(im_sag, [512 512]);
small_pic2 = imresize(im_cor, [512 512]);
small_pic3 = imresize(im_axi, [512 512]);

% small_pic1(1:min(800,size(im_sag,1)),1:min(800,size(im_sag,2)),:) = im_sag(1:min(800,size(im_sag,1)),1:min(800,size(im_sag,2)),:);
% small_pic2(1:min(800,size(im_cor,1)),1:min(800,size(im_cor,2)),:) = im_cor(1:min(800,size(im_cor,1)),1:min(800,size(im_cor,2)),:);
% small_pic3(1:min(800,size(im_axi,1)),1:min(800,size(im_axi,2)),:) = im_axi(1:min(800,size(im_axi,1)),1:min(800,size(im_axi,2)),:);
all_pics = cat(2,small_pic1,small_pic2,small_pic3);
text_pic = zeros(500,size(all_pics,2),3);

% volumes in cm3
spleen_vol=sum(SegNii.img(:)==1)*prod(voxdim)/1000;

% %masi logo
% masi_log_file = 'masi_logo.png';
% if ~exist(masi_log_file)
%     masi_log_file = '/extra/masi_logo.png';
% end
% masi_log = imread(masi_log_file);
% masi_log = double(masi_log)./255;
% % masi_log = imresize(masi_log,[240,300]);
% text_pic(1:size(masi_log,1),end-size(masi_log,2)+1:end,:) = masi_log;

[~,rundate]=system('date');

text_fname = strrep(pdffn,'.jpg','.txt');
fp = fopen(text_fname,'w+');
fprintf(fp,'Spleen Volume = %.2f cc\n',spleen_vol);
fprintf(fp,'Not for clinical Use. Use of this service is for technical demonstration only\n');
fprintf(fp,'Docker Name:      DeepSpleenSeg-v2017-11-8\n');
fprintf(fp,'Demo Produced By: Yuankai Huo, Hyeonsoo Moon, Richard G. Abramson, Bennett A. Landman\n');
fprintf(fp,'Citation:         Yuankai Huo et al."Splenomegaly Segmentation using Global Convolutional Kernels and\n');
fprintf(fp,'                  Conditional Generative Adversarial Networks" SPIE Medical Imaging 2018\n');
fprintf(fp,'Contact:          bennett.landman@vanderbilt.edu\n');
fprintf(fp,'Date of Run:      %s\n',rundate);
fprintf(fp,'Docker Name:      DeepSpleenSeg-v2017-11-8\n');
fclose(fp);

% position = [0 10];
% value = sprintf('Spleen Volume = %.2f cc',spleen_vol);

% 
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 45, 'BoxColor', 'black', 'TextColor', 'red', 'BoxOpacity', 0);
% position = position + [0 100];
% value = sprintf('Not for clinical Use. Use of this service is for technical demonstration only');
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 33, 'BoxColor', 'black', 'TextColor', 'yellow', 'BoxOpacity', 0);
% position = position + [0 100];
% value = sprintf('Docker Name:      DeepSpleenSeg-v2017-11-8');
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 30, 'BoxColor', 'black', 'TextColor', 'white', 'BoxOpacity', 0.6);
% position = position + [0 40];
% value = sprintf('Demo Produced By:      Yuankai Huo, Hyeonsoo Moon, Richard G. Abramson, Bennett A. Landman');
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 30, 'BoxColor', 'black', 'TextColor', 'white', 'BoxOpacity', 0.6);
% 
% position = position + [0 40];
% value = sprintf('Citation:      Yuankai Huo et al."Splenomegaly Segmentation using Global Convolutional Kernels and');
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 30, 'BoxColor', 'black', 'TextColor', 'white', 'BoxOpacity', 0.6);
% position = position + [0 40];
% value = sprintf('                   Conditional Generative Adversarial Networks" SPIE Medical Imaging 2018');
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 30, 'BoxColor', 'black', 'TextColor', 'white', 'BoxOpacity', 0.6);
% position = position + [0 40];
% value = sprintf('Contact:      bennett.landman@vanderbilt.edu');
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 30, 'BoxColor', 'black', 'TextColor', 'white', 'BoxOpacity', 0.6);
% 
% position = position + [0 40];
% value = sprintf('Date of Run:      %s',rundate);
% text_pic= insertText_yh(text_pic,position,value, 'FontSize', 30, 'BoxColor', 'black', 'TextColor', 'white', 'BoxOpacity', 0.6);

final_pic = all_pics;
% imagesc(final_pic)

imwrite(final_pic,pdffn);
...
ns=et(1)+et(3);
t_DemoTester_content=text(ns,0.30,...
    'Hyeonsoo Moon, Yuankai Huo, Bennett A. Landman, Rick Abramson',...
    'fontsize',9,...
    'interpreter','none');
