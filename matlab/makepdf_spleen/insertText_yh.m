function RGB = insertText_yh(I, position, text, varargin)
...
if isSimMode()
    fontFileName = '/extra/LucidaSansRegular.ttf';
    faceIndex = 0;
    if ~exist(fontFileName)
...
%   defaults = getDefaultParameters(inpClass);
  defaults.AnchorPoint = 'LeftTop';
  defaults.TextColor = [0 0 0];
  defaults.BoxColor = [1 1 0];
  defaults.BoxOpacity = 0.6000;
  defaults.Font = 'LucidaSansRegular';
  defaults.FontSize = 12;
  defaults.ShapeWidth = 0;
  defaults.ShapeHeight = 0;
...
font        = 'LucidaSansRegular';%checkFont(parser.Results.Font);% readjusted case in font name
